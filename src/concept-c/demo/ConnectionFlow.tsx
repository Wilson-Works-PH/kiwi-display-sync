...
export function ConnectionFlow({
  ...
  return (
    <>
      ...
      <div
        className="relative hidden h-[320px] w-[96px] shrink-0 self-center lg:block"
        aria-hidden="true"
      >
        <svg
          width={W}
        ...
        >
          <path
            ...
            stroke="#7fa060"
            strokeWidth="1.5"
            className={cx("c-flow-line", busy && "is-busy")}
          />
          {OUT_PATHS.map((d) => (
            <path
              key={d}
        ...
        <div className="absolute left-1/2 top-1/2 grid size-9 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-white text-plum-950 shadow-[0_8px_20px_-8px_rgba(45,13,41,0.5)] ring-1 ring-plum-950/10">
          <span className="c-icon is-filled" style={{ fontSize: 20 }}>
            cloud
          </span>
        </div>
        {busy
          ? OUT_PATHS.map((d, i) => (
              <span
                ...
                style={
                  {
                    offsetPath: `path('${IN_PATH.replace("L 40 160", "L 48 160")} ${d.replace("M 56 160", "L 56 160")}')`,
      ...
      <div
        className="flex items-center justify-center gap-3 py-2 text-[12px] font-semibold text-plum-950/60 lg:hidden"
        aria-hidden="true"
      >
        <span>Dashboard</span>
        <span
          className={cx(
            "h-px w-10 border-t border-dashed border-leaf-600",
            busy && "border-lime-400",
          )}
        />
        <span className="grid size-8 place-items-center rounded-full bg-white text-plum-950 shadow ring-1 ring-plum-950/10">
          <span className="c-icon is-filled" style={{ fontSize: 18 }}>
            cloud
          </span>
        </span>
        <span
